...
def main(simulado):
    ...
    f_resultados = pd.read_sql(f"SELECT * FROM {SCHEMA}.f_resultados WHERE presenca_id = '0';", engine)
    f_resultados_respostas = pd.read_sql(f"SELECT * FROM {SCHEMA}.f_resultados_respostas;", engine)

    filtered_f_resultados_respostas = f_resultados_respostas[
    f_resultados_respostas['resultado_id'].isin(f_resultados['resultado_id'])
    ]


    f_resultados_respostas = corrigir_respostas(filtered_f_resultados_respostas, gabarito)
